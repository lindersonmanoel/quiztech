...
- **Banco:** PostgreSQL. Em produção, no **Neon**; em Docker local, o mesmo servidor do Meu Bolso Digital.
- **Publicação:** o site e a API rodam no **Vercel** (São Paulo) e o banco é PostgreSQL no **Neon** (São Paulo): nada depende do computador do dono (veja [DEPLOY.md](DEPLOY.md), seção 5d). Continua possível rodar em Docker numa VM própria (seções anteriores). CI no GitHub.
